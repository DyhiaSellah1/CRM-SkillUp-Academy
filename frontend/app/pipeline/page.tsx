...
export default function PipelinePage() {
    ...
    const loadPipeline = async () => {
        try {
            const [boardRes, contactsRes, stagesRes] = await Promise.all([
                fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/pipeline-board`),
                fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/contacts`),
                fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/pipeline-stages`),
            ]);
    ...
    const totalPipeline = useMemo(() => {
        return board.reduce((sum, stage) => {
            return (
                sum +
                stage.leads.reduce(
                    (stageSum, lead) => stageSum + Number(lead.amount || 0),
                    0
                )
            );
        }, 0);
    ...
    const getStageColor = (label: string) => {
        ...
        if (value.includes("négociation") || value.includes("negociation"))
            return "bg-orange-500";
        if (value.includes("gagné") || value.includes("gagne"))
    ...
    const handleDropLead = async (leadId: number, newStageId: number) => {
        const oldBoard = board.map((stage) => ({
            ...stage,
            leads: [...stage.leads],
        }));
        ...
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/api/leads/${leadId}/stage`,
                {
                    method: "PATCH",
                    headers: {
        ...
        <div className="min-h-screen bg-[#f5f7fb] p-6">
            ...
            <div className="overflow-x-auto pb-4">
                <div className="flex min-w-max gap-6">
                    ...
                    <span
                        className={`h-3 w-3 rounded-full ${getStageColor(
                            stage.label
                        )}`}
                    />
